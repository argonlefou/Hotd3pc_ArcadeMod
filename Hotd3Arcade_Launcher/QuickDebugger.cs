...
namespace QuickDebugger
{
    public class QuickDebugger
    {
        ...
        {
            //DebugActiveProcessStop((uint)dbe.dwProcessId);
                    ...
                    case DebugEventType.CREATE_PROCESS_DEBUG_EVENT:
                        {
                            ...
                            ContinueDebugEvent(dbe.dwProcessId, dbe.dwThreadId, ContinueStatus.DBG_CONTINUE);
                        } break;
                    ...
                    case DebugEventType.CREATE_THREAD_DEBUG_EVENT:
                        {
                            ...
                            ti = dbe.CreateThread;
                            //Logger.WriteLog("Thread 0x" + ti.hThread.ToString("X8") + " (Id: " + dbe.dwThreadId.ToString() + ") created");
                            ContinueDebugEvent(dbe.dwProcessId, dbe.dwThreadId, ContinueStatus.DBG_CONTINUE);
                        } break;
                    ...
                    case DebugEventType.EXCEPTION_DEBUG_EVENT:
                        {
                            ...
                            if (Ex.ExceptionRecord.ExceptionCode == STATUS_BREAKPOINT)
                            {
                                ...
                                Process p = Process.GetProcessById(dbe.dwProcessId);
                                _TargetProcess = p;
                                ...
                                //Now that the job is done, detach from the game so that iterator can run on i's own
                                DebugActiveProcessStop((uint)dbe.dwProcessId);
                                _ContinueDebugging = false;
                            }
                        } break;
    ...
    {
        public QuickDebugger.DEBUG_EVENT e { get; private set; }

        public DebugEventArgs(QuickDebugger.DEBUG_EVENT DebugEvent)
        {
            e = DebugEvent;
        }
    }
